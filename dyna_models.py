...
class CustomCell(nn.Module):
    def __init__(self, embedding_size, feature_size):
        ...
        self.WdynU1 = torch.nn.Parameter(embedding_size, self.uembsz, self.N)
        self.WdynU2 = torch.nn.Parameter(embedding_size, self.uembsz, self.N)
        self.WdynI1 = torch.nn.Parameter(embedding_size, self.iembsz, self.N)
        self.WdynF  = torch.nn.Parameter(embedding_size, self.featsz, self.N)
        self.WdynT  = torch.nn.Parameter(embedding_size, 1, self.N)
    
    def forward(self, U1, U2, I1, F, T):
        ntensor = torch.cat([U1, U2, I1, F, T])
        ...
        W4 = torch.mm(self.WdynI1.view(self.embsz * self.featsz, -1), ntensor).view(self.embsz, self.featsz, -1)
        W5 = torch.mm(self.WdynI1.view(self.embsz * 1, -1), ntensor).view(self.embsz, 1, -1)
        
        ### All N_is are of same size
        N_1 = torch.tanh(torch.mm(W1.squeeze(), U1))
        N_2 = torch.tanh(torch.mm(W2.squeeze(), U2))
        N_3 = torch.tanh(torch.mm(W3.squeeze(), U3))
        N_4 = torch.tanh(torch.mm(W4.squeeze(), U4))
        N_5 = torch.tanh(torch.mm(W5.squeeze(), U5))
        
        # return the sum of all N_i's
        return torch.sum(torch.stack([N_1, N_2, N_3, N_4, N_5], dim=1), dim=1)
...
class Wrapper(nn.Module):
    ...
    def forward(self, BU1, BU2, BI1, BF, BT):
        ...
        for ue, oe, ie, fv, delta in zip(torch.unbind(BU1, 0), torch.unbind(BU2, 0), torch.unbind(BI1, 0), torch.unbind(BF, 0), torch.unbind(BT, 0)):
            new_embedding = self.cell(ue, oe, ie, fv, delta)
